...
import edu.usst.jayce.server.exception.FilesTotalOutOfLimitException;
...
import edu.usst.jayce.server.util.FileNodeUtil;
...
import edu.usst.jayce.util.file_system_manager.pojo.FolderView;

//命令模式启动器

public class ConsoleRunner {
	private static ConsoleRunner cs;
	private static SystemCtl ctl;
	...
	// 执行相应的指令并进行后续处理，该方法为整个CloudDisk命令模式的起点。
	private void execute(final String[] args) {
		if (args.length > 0) {
			...
			default: {
				Printer.instance.print("system:无效的指令，使用控制台模式启动请输入参数 -console，直接启动服务器引擎请输入参数 -start，使用UI模式启动请不要传入任何参数。");
				break;
			}
			}
		}
	...
	private void startKiftdByConsole() {
		Printer.instance.print(" CloudDisk-system 控制台模式[Console model]");
	...
	private void startServer() {
		...
		if (ConsoleRunner.ctl.started()) {
			...
			Printer.instance.print("CloudDisk服务器已启动，可以正常访问了，您可以使用 -status 指令查看运行状态。");
		} else {
			if (ConfigureReader.instance().getPropertiesStatus() != 0) {
				switch (ConfigureReader.instance().getPropertiesStatus()) {
				case ConfigureReader.INVALID_PORT:
					Printer.instance.print("错误：CloudDisk服务器未能启动，端口设置无效。");
					break;
				case ConfigureReader.INVALID_BUFFER_SIZE:
					Printer.instance.print("错误：CloudDisk服务器未能启动，缓存设置无效。");
					break;
				case ConfigureReader.INVALID_FILE_SYSTEM_PATH:
					Printer.instance.print("错误：CloudDisk服务器未能启动，文件系统路径或某一扩展存储区设置无效。");
					break;
				case ConfigureReader.INVALID_LOG:
					Printer.instance.print("错误：CloudDisk服务器未能启动，日志设置无效。");
					break;
				case ConfigureReader.INVALID_VC:
					Printer.instance.print("错误：CloudDisk服务器未能启动，登录验证码设置无效。");
					break;
				default:
					Printer.instance.print("错误：CloudDisk服务器未能启动，请重试或检查设置。");
					break;
				}
			} else {
				Printer.instance.print("错误：CloudDisk服务器未能启动，请重试或检查设置。");
	...
	private void exit() {
		Printer.instance.print("执行命令：停止服务器并退出CloudDisk...");
	...
	private void awaiting() {
		Thread t = new Thread(() -> {
			...
			try {
				while (true) {
					Printer.instance.print("CloudDisk: console$ ");
				...
				Printer.instance.print("错误：读取命令时出现意外导致程序退出，请重启CloudDisk。");
			}
		});
		t.start();
	}
	...
	private void fileSystemManagerModel() {
		...
		try {
			...
			Printer.instance.print("错误：无法打开文件系统，该文件系统可能正在被另一个CloudDisk占用。");
			return;
		}
		System.out.println("命令帮助：\r\n" + fsCommandTips + "\r\n");
		try {
			while (true) {
				System.out.println("CloudDisk: " + currentFolder.getCurrent().getFolderName() + "$ ");
				String command = reader.nextLine();
				// 针对一些带参数指令的操作
		...
		while (true) {
			...
			case "Y":
				try {
					...
					Printer.instance.print("错误：无法读取根目录，请尝试重新打开文件管理系统或重启CloudDisk。");
				}
				return;
			case "N":
				return;
