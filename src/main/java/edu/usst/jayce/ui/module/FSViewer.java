...
import edu.usst.jayce.ui.util.FilesTable;
...
public class FSViewer extends KiftdDynamicWindow {
	...
	private FSViewer() throws SQLException {
		...
		window = new JDialog(ServerUIModule.window, "CloudDisk-ROOT");
	...
	public void show() {
		...
		try {
			...
			// TODO 自动生成的 catch 块
			Printer.instance.print("错误：无法打开文件系统，该文件系统可能正在被另一个服务器占用。");
		}
	}
	...
	private void getFolderView(String folderId) throws Exception {
		try {
			...
			if (currentView != null && currentView.getCurrent() != null) {
				...
				window.setTitle("CloudDisk-" + currentView.getCurrent().getFolderName());
			} else {
				// 浏览一个不存在的文件夹时自动返回根目录
				getFolderView("root");
			}
