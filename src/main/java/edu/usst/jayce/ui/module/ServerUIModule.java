...
import edu.usst.jayce.ui.callback.*;
...
public class ServerUIModule extends KiftdDynamicWindow {
	...
	private ServerUIModule() throws Exception {
		setUIFont();
		(ServerUIModule.window = new JFrame("CloudDisk-服务器控制台")).setSize(OriginSize_Width, OriginSize_Height);
		ServerUIModule.window.setLocation(100, 100);
		ServerUIModule.window.setResizable(false);
		try {
			ServerUIModule.window.setIconImage(
					ImageIO.read(this.getClass().getResourceAsStream("/icon.png")));
		...
		final JLabel title = new JLabel("CloudDisk");
		...
		final JLabel subtitle = new JLabel("校园云盘-服务器");
		...
		bottombox.add(new JLabel("--CloudDisk--"));
					...
					Thread t = new Thread(() -> {
						if (ss.start()) {
							...
							if (st.getServerStatus()) {
								printMessage("CloudDisk服务器已经启动，可以正常访问了。");
							} else {
								printMessage("CloudDisk服务器未能成功启动，请检查设置或查看异常信息。");
							}
						} else {
							if (ConfigureReader.instance().getPropertiesStatus() != 0) {
								...
								case ConfigureReader.INVALID_PORT:
									printMessage("CloudDisk无法启动：端口设置无效。");
									break;
								case ConfigureReader.INVALID_BUFFER_SIZE:
									printMessage("CloudDisk无法启动：缓存设置无效。");
									break;
								case ConfigureReader.INVALID_FILE_SYSTEM_PATH:
									printMessage("CloudDisk无法启动：文件系统路径或某一扩展存储区设置无效。");
									break;
								case ConfigureReader.INVALID_LOG:
									printMessage("CloudDisk无法启动：日志设置无效。");
									break;
								case ConfigureReader.INVALID_VC:
									printMessage("CloudDisk无法启动：登录验证码设置无效。");
									break;
								case ConfigureReader.INVALID_MUST_LOGIN_SETTING:
									printMessage("CloudDisk无法启动：必须登入设置无效。");
									break;
								case ConfigureReader.INVALID_CHANGE_PASSWORD_SETTING:
									printMessage("CloudDisk无法启动：用户修改账户密码设置无效。");
									break;
								case ConfigureReader.INVALID_FILE_CHAIN_SETTING:
									printMessage("CloudDisk无法启动：永久资源链接设置无效。");
									break;
								default:
									printMessage("CloudDisk无法启动，请检查设置或查看异常信息。");
									break;
								}
							} else {
								printMessage("CloudDisk无法启动，请检查设置或查看异常信息。");
				...
				Thread t = new Thread(() -> {
					if (cs != null) {
						...
						if (cs.close()) {
							...
							if (st.getServerStatus()) {
								printMessage("CloudDisk服务器未能成功关闭，如有需要，可以强制关闭程序（不安全）。");
							} else {
								printMessage("CloudDisk服务器已经关闭，停止所有访问。");
							}
						} else {
							printMessage("CloudDisk服务器无法关闭，请手动结束本程序。");
						}
						updateServerStatus();
					}
				});
