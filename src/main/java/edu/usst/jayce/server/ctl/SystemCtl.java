package edu.usst.jayce.server.ctl;

import edu.usst.jayce.printer.Printer;
import edu.usst.jayce.server.configation.MVC;
...
public class SystemCtl {
		...
		if (!SystemCtl.run) {
			...
			if (ConfigureReader.instance().getPropertiesStatus() == 0) {
				try {
					...
					SpringApplication springApplication = new SpringApplication(SystemCtl.class);
					springApplication.setBannerMode(Banner.Mode.OFF);// 关闭自定义标志输出，简化日志信息
					SystemCtl.context = springApplication.run(args);
					SystemCtl.run = (SystemCtl.context != null);
					Printer.instance.print("服务器引擎已启动。");
					return SystemCtl.run;
				} catch (Exception e) {
					return false;
				}
			}
	...
	public boolean stop() {
		...
		if (SystemCtl.context != null) {
			Printer.instance.print("正在终止服务器引擎...");
			try {
				SystemCtl.run = (SpringApplication.exit(SystemCtl.context, new ExitCodeGenerator[0]) != 0);
