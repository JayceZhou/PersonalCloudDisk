...
@Component
public class DFFMPEGLocator implements ProcessLocator {
	...
	private String initFFMPEGExecutablePath() {
		...
		// 对于类Unix系统而言，还要确保临时目录授予可运行权限，以便java运行时调用ffmpeg
		if (!isWindows) {
		...
		// 上述工作都做好了，就可以将ffmpeg的路径返回给java调用了。
		// 如果到不了这里，说明初始化失败，该方法返回null，那么应该禁用java的在线转码功能
		enableFFmpeg = true;
